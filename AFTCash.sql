...
--
-- Host: localhost
-- Generation Time: Mar 30, 2018 at 06:50 AM
...
-- --------------------------------------------------------
--
-- Table structure for table `sale' which calculate how many items sold with categoryId.
--
CREATE TABLE sale
	(`CategoryId` int, `Quantity` decimal(11, 3));
-- --------------------------------------------------------
--
-- Table structure for table `inventory level' to check how many of each items we have in stock.`
--
CREATE TABLE invlevel
	(`CategoryId` int, `Quantity` decimal(11, 3));
--
-- dumping data into invelevel. 
--

INSERT INTO invlevel
	(`CategoryId`, `Quantity`)
VALUES
	(1, 100),
	(2, 120),
...
--
-- create table reorder so this info can used whenever an item is out of stock and ordering again.
--

CREATE TABLE reorder
	(`ProductID` int, `Amount` decimal(11, 3));
--
-- dumping data into reorder. 
--
INSERT INTO reorder
	(`CategoryId`, `Amount`)
VALUES
...
-- --------------------------------------------------------
--
-- craeting table 'orders'.This will take data from reorder and shows which item need to order according to the trigger.
--
...
-- --------------------------------------------------------
--
-- creating trigger. This executes after inserting somthing to 'sale' and checks inv level.
--
...
-- --------------------------------------------------------
--
-- creating trigger, This executes after updating 'invlevel' collects data from 'reorder' table if any item quantity has less than 20 left and need to order. 
--
CREATE TRIGGER invlevel_check
AFTER UPDATE ON invlevel 
FOR EACH ROW
  INSERT INTO orders (CategoryId, Amount)
...
-- --------------------------------------------------------
--
-- dumping data into 'sale'.Trigger sale_item will execute after this insert statement. 
--
INSERT INTO sale
	(`CategoryId`, `Quantity`)
VALUES
